...
class myset{
    ...
    int lower_bound(T val,np t){
        if(!t)return 0;
        bool b=val>t->val;
        return(b?count2(t->ch[0])+1:0)+lower_bound(val,t->ch[b]);
    }
	int upper_bound(T val,np t){
        if(!t)return 0;
        bool b=val>=t->val;
        return(b?count2(t->ch[0])+1:0)+upper_bound(val,t->ch[b]);
    }
    T find_by_order(int idx,np t){
		if(idx==count2(t->ch[0]))return t->val;
        bool b=idx>count2(t->ch[0]);
		return find_by_order(idx-(b?count2(t->ch[0])+1:0),t->ch[b]);
	}
    public:

...
template <typename Node> struct splay_tree_base {
    using np = Node*;
    np root = 0;
    ...
    template <typename C> int lower_bound(C less) {
        int res = __lower_bound(root, less);
        if (res < size()) root = splay(root, res);
        return res;
    }
    template <typename C> int __lower_bound(np t, C less) {
        if (!t) return 0;
        bool b = less(t);
        if (b)
            return size(t->ch[0]) + 1 + __lower_bound(t->ch[1], less);
        else
            return __lower_bound(t->ch[0], less);
    }
};
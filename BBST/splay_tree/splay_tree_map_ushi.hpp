...
template <typename T, typename E, typename F> struct splay_tree_map_ushi_node {
    ...
    E sum;
    F op;
    splay_tree_map_ushi_node(T key, E val, F op)
        : key(key), val(val), sum(val), op(op) {}
...
struct splay_tree_map_ushi
    : splay_tree_base<splay_tree_map_ushi_node<T, E, F>> {
    ...
    int get_idx(T key) {
        return super::lower_bound([&](np t) { return t->key < key; });
    }
    ...
    template <typename G> void update(T key, G g) {

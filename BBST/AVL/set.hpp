...
template <typename T> struct avl_set : public AVL_base<Node<T>> {
    using AVL_base<Node<T>>::AVL_base;
    using np = Node<T>*;
    np root = 0;
    int lower_bound(np t, np x) {
        if (*x < *t)
            return lower_bound(t->ch[0], x);
        else
            return AVL_base<Node<T>>::sz(t->ch[0]) + 1 +
                   lower_bound(t->ch[1], x);
    }
    int upper_bound(np t, np x) {
        if (*x <= *t)
            return upper_bound(t->ch[0], x);
        else
            return AVL_base<Node<T>>::sz(t->ch[0]) + 1 +
                   upper_bound(t->ch[1], x);
    }
    int count(T x) {
        np t = new Node<T>(x);

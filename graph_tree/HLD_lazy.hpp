...
class HLD_lazy {
  ...
  public:
    HLD_lazy(const graph& v, int root = 0, F f = F(), G g = G(), H h = H())
        : _f(f), _g(g), _h(h) {
        make(v, root);
        seg = new lazy_segment_tree<T, E, F, G, H>(v.size(), f, g, h);
    }
    // HLD_lazy(const graph& v,const vector<T>& a,int root=0,F f=F(),G g=G(),H
    // h=H()):_f(f),_g(g),_h(h){ 	vector<T>tmp(v.size()); 	make(v,root); 	for(int
    // i=0;i<(int)v.size();i++){ 		tmp[vertex[i]]=a[i];
    // 	}
    ...
    inline maybe<T> get_vertex(int u) { return get_seg(vertex[u], vertex[u]); }

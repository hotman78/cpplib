...
template <typename T, typename F> struct tree_convolution {
    graph g;
    vector<T> st;
    F f;
    centroid_decomposition* cd;
    tree_convolution(const graph& g, vector<T> st, F f = F())
        : g(g), st(st), f(f) {
        cd = new centroid_decomposition(g);
    }
    T run() { return dfs(cd->get_root()); }
    T dfs(int now) {
        T res = st[now];
        set<int> s;

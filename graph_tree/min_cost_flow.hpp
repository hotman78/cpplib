...
template <typename T, typename Res = T> struct min_cost_flow {
    int v;
    Res ans = 0;
    struct edge {
        int to;
    ...
    void add_edge(int from, int to, T mn, T mx, T cost) {
        ...
        if (from == to) {
            if (cost < 0)
                res[idx++] = mx, ans += mx * cost;
            else
                res[idx++] = mn, ans += mn * cost;
            return;
        }
        if (cost >= 0) {
            e[from].push_back(new edge{to, mx - mn, cost * v, mn,
                                       (int)e[to].size(), 0, 0, idx});
            e[to].push_back(new edge{from, 0, -cost * v, mn,
                                     (int)e[from].size() - 1, 1, 0, idx++});
            ans += mn * cost;
            d[from] -= mn;
            d[to] += mn;
            ...
            e[to].push_back(new edge{from, mx - mn, -cost * v, mx,
                                     (int)e[from].size(), 0, 1, idx});
            e[from].push_back(new edge{to, 0, cost * v, mx,
                                       (int)e[to].size() - 1, 1, 1, idx++});
            ans += mx * cost;
            d[from] -= mx;
            d[to] += mx;
            eps = max(eps, -cost * v * v);
    ...
    Res flow() {
        for (; eps; eps >>= 1) {
            ...
            while (!active.empty()) {
                ...
                for (auto ed : e[node]) {
                    if (!d[node]) break;
                    if (-eps <= ed->cost - p[node] + p[ed->to] &&
                        ed->cost - p[node] + p[ed->to] < 0) {
    ...
    std::vector<T> flow_result() {
        for (int i = 0; i < v; i++)
            for (auto ed : e[i]) {
                if (ed->is_rev) continue;
                res[ed->id] =
                    ed->st + e[ed->to][ed->rev]->cap * (ed->edge_rev ? -1 : 1);
            }
        return res;
    }
    // flow_resultを渡す

template <typename T> struct Dirichlet {
    ...
    P& operator*=(const P& x) {
        ...
        auto getB = [&](long long n) {
            return (n <= K ? sum_b[n] : x.A[N / n]);
        };
        for (int i = 1; i <= L; ++i) {
    ...
    P& operator/=(const P& x) {
        ...
        auto getA = [&](long long n) {
            return (n <= K ? sum_a[n] : x.A[N / n]);
        };
        auto getB = [&](long long n) {
            return (n <= K ? sum_b[n] : res.A[N / n]);
        };
        for (int i = L; i >= 1; --i) {
            const long long n = N / i, m = std::floor(std::sqrt(n));
            for (int j = 1; j <= m; ++j) {
    ...
    template <typename F, typename G> static P make(F f, G g) {
        P res;
        ...
        auto get = [&](long long n) {
            ...
            if (tmp2 == T()) {
                for (int i = 0; i <= k + 1; ++i) {
                    ...
                    res += tmp[i] * fact_inv[i] * fact_inv[k + 1 - i] * tmp3 *
                           ((k + 1 - i) % 2 ? T(-1) : T(1));
                }
            } else {
                for (int i = 0; i <= k + 1; ++i) {
                    res += tmp[i] * fact_inv[i] * fact_inv[k + 1 - i] * tmp2 /
                           T(n - i) * ((k + 1 - i) % 2 ? T(-1) : T(1));
                }
            }
            return res;
        };
    ...
    // σ_k(s)=ζ(s)ζ(s-k)
    static P divisor(int k) { return zeta() * zeta_shift(k); }
    // φ(s)=ζ(s-1)/ζ(s)
    static P euler() { return zeta_shift(1) / zeta(); }

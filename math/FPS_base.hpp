...
template <typename T, typename F> struct FPS_BASE : std::vector<T> {
    ...
    inline P& shrink() {
        while ((int)(*this).size() != 1 && (*this).back() == 0)
            (*this).pop_back();
        return (*this);
    }
    ...
    P pow(int c, int deg = -1) {
        ...
        T k = ret[i];
        return ((((ret >> i) / k).log(n) * c).exp(n) * (k.pow(c)) << (i * c))
            .pre(n);
        // const int n=deg==-1?this->size():deg;
        // long long i=0;
        // P ret(*this);
    ...
    static P interpolation(const std::vector<T>& x, const std::vector<T>& y) {
        ...
        for (int i = 0; i < n; ++i)
            a[i + n - 1] = std::make_pair(P{1}, P{T() - x[i], 1});
        for (int i = n - 2; i >= 0; --i)
            a[i] = {a[2 * i + 1].first * a[2 * i + 2].second +
                        a[2 * i + 2].first * a[2 * i + 1].second,
                    a[2 * i + 1].second * a[2 * i + 2].second};
        auto d = (a[0].first).multipoint_eval(x);
        for (int i = 0; i < n; ++i) b[i + n - 1] = P{T(y[i] / d[i])};
        for (int i = n - 2; i >= 0; --i)
            b[i] = b[2 * i + 1] * a[2 * i + 2].second +
                   b[2 * i + 2] * a[2 * i + 1].second;
        return b[0];
    }
    static P interpolation(const std::vector<T>& y) {
        ...
        std::vector<P> b(n * 2 - 1);
        for (int i = 0; i < n; ++i)
            a[i + n - 1] = std::make_pair(P{1}, P{T() - i, 1});
        for (int i = n - 2; i >= 0; --i)
            a[i] = {a[2 * i + 1].first * a[2 * i + 2].second +
                        a[2 * i + 2].first * a[2 * i + 1].second,
                    a[2 * i + 1].second * a[2 * i + 2].second};
    ...
    T nth_term(P q, int64_t x) {
        ...
        q *= q2;
        p *= q2;
        return p.slice(x % 2, p.size(), 2)
            .nth_term(q.slice(0, q.size(), 2), x / 2);
    }
    T guess(int64_t x) {
    ...
    P gcd(P q) { return *this == P() ? q : (q % (*this).shrink()).gcd(*this); }
    //(*this)(t(x))
    P manipulate(P t, int deg) {
        ...
        int k =
            std::min((int)::sqrt(deg / (::log2(deg) + 1)) + 1, (int)t.size());
        int b = deg / k + 1;
        P t2 = t.pre(k);

...
template <typename X = long long,
          ...
          typename F = std::plus<T>,
          typename Inv = minus<T>>
std::vector<T> static_rectangle_sum(const vector<tuple<X, Y, T>>& p,
                                    const vector<tuple<X, Y, X, Y>>& query,
                                    T zero = T(),
                                    F f = F(),
                                    Inv inv = Inv()) {
    ...
    for (int i = 0; i < n; ++i) {
        auto y = std::lower_bound(idx.begin(), idx.end(), std::get<1>(p[i])) -
                 idx.begin();
        event.emplace_back(std::make_pair(get<0>(p[i]), 2),
                           Point{y, get<2>(p[i])});
    }
    for (int i = 0; i < q; ++i) {
        auto l =
            std::lower_bound(idx.begin(), idx.end(), std::get<1>(query[i])) -
            idx.begin();
        auto r =
            std::lower_bound(idx.begin(), idx.end(), std::get<3>(query[i])) -
            idx.begin();
        event.emplace_back(std::make_pair(get<0>(query[i]), 0), Query{l, r, i});
        event.emplace_back(std::make_pair(get<2>(query[i]), 1),
                           Query{l, r, i + q});
    }
    sort(event.begin(), event.end(),
         [](const auto& s, const auto& t) { return s.first < t.first; });

template <typename T, typename E> class link_cut {
    class myset {
        struct node {
            T val, l, r, sum;
            int cnt = 1, dep = 0;
        ...
        np update(np t, bool flag = 1) {
            ...
            if (flag && abs(depth(t->ch[0]) - depth(t->ch[1])) == 2) {
                ...
                if (t->ch[b] &&
                    depth(t->ch[b]->ch[b]) < depth(t->ch[b]->ch[1 - b])) {
                    t->ch[b] = rot(t->ch[b]);
                }
                t = rot(update(t, 0));
            }
        ...
        E sum(T l, T r, np t) {
            ...
            return f(sum(l, r, t->ch[0]),
                     f(b ? t->val : et, sum(l, r, t->ch[1])));
        }
      ...
      public:
        ...
        inline int count(T val) {
            return upper_bound(val, root) - lower_bound(val, root);
        }
        inline int count(T l, T r) {
            return lower_bound(r, root) - lower_bound(l, root);
        }
  ...
  public:
    ...
    struct node {
        ...
        node() {}
        node(int idx, T key)
            : idx(idx), key(key), sum(key), sum2(et), csum(et), sz(1) {}
    ...
    void update(np t) {
        ...
        if (t->ch[1]) t->sum = f(t->sum, t->ch[1]->sum);
        t->csum =
            f(f(t->key, t->light.all_sum()), t->ch[1] ? t->ch[1]->sum2 : et);
        t->sum2 = f(f(f(t->ch[0] ? t->ch[0]->sum2 : et, t->key),
                      t->ch[1] ? t->ch[1]->sum2 : et),
                    t->light.all_sum());
    }
    ...
    static T et;
    static E ee;
    constexpr static T s(T s) { return s; }
    constexpr static T f(T s, T t) { return s + t; }
    constexpr static T g(T s, E t, int len) { return s + t * len; }
    constexpr static E h(E s, E t) { return s + t; }
  ...
  public:
    ...
    T get_key(int t) { return p[t]->key; }
    ...
    void make_node(T x) { p.emplace_back(new node(p.size(), x)); }
    void evert(int t) { evert(p[t]); }
    ...
    bool same(int s, int t) { return get_root(p[s]) == get_root(p[t]); }
    np lca(int s, int t) {

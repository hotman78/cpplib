...
template <typename T> class LHT {
    ...
    np update(T a, T b, lint p, lint q, lint l, lint r, np t) {
        ...
        if (p <= l && r <= q) {
            ...
            if (sign((a - t->a) * l + (b - t->b)) *
                    sign((a - t->a) * m + (b - t->b)) <=
                0)
                t->ch[0] = update(a, b, p, q, l, m, t->ch[0]);
            if (sign((a - t->a) * m + (b - t->b)) *
                    sign((a - t->a) * r + (b - t->b)) <=
                0)
                t->ch[1] = update(a, b, p, q, m, r, t->ch[1]);
            return t;
        }
        // 一部区間内
    ...
    T get(lint x, lint l, lint r, np t) {
        ...
        lint m = (l + r) / 2;
        if (l <= x && x < m)
            return std::min(t->a * x + t->b, get(x, l, m, t->ch[0]));
        else
            return std::min(t->a * x + t->b, get(x, m, r, t->ch[1]));
    }
    lint mn;
    lint mx;
  ...
  public:
    ...
    T get(lint x) { return get(x, mn, mx, root); }
    void add_line(T a, T b) { root = update(a, b, mn, mx, mn, mx, root); }

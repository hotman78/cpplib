template <typename T, typename E> class HLD_lazy {
    class lazy_segment_tree {
      ...
      public:
        ...
        void update(auto l, auto r, E val) {
            ...
            while (l < r) {
                if (l & 1) {
                    lazy[l - 1] = g(lazy[l - 1], val);
                    eval(l - 1);
                    ++l;
                }
                if (r & 1) {
                    --r;
                    lazy[r - 1] = g(lazy[r - 1], val);
                    eval(r - 1);
                }
                l >>= 1;
                r >>= 1;
            ...
            for (int i = 0; i < idx; i++) {
                ...
                eval((ids[i] << 1) + 2);
                node[ids[i]] =
                    f(node[(ids[i] << 1) + 1], node[(ids[i] << 1) + 2]);
            }
        }
        T get(auto l, auto r) {
            if (l > r) swap(l, r);
  ...
  public:
    ...
    inline T get_vertex(auto u) { return seg->get(vertex[u], vertex[u]); }
  ...
  protected:
    ...
    constexpr static T f(const T& a, const T& b) { return a + b; }
    constexpr static T h(const T& a, const E& b, const auto& sz) {
        return a + b * sz;
    }
    constexpr static E g(const E& a, const E& b) { return a + b; }
    constexpr static auto update(auto a, auto b) { return b == ee ? a : b; }
};
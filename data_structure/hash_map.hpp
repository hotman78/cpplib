...
template <typename T, typename E, typename F = std::hash<T>> struct hash_map {
    struct node {
        T key;
        E val;
        unsigned long b;
    ...
    size_t sz;
    node* point;
    constexpr static size_t mod_list[24] = {
        37,      67,       131,      257,      521,       1031,
    ...
    node** table;
    F op;
    hash_map(F op = F()) : sz(15), op(op) { table = new node*[mod_list[sz]](); }
    node** _realloc() {
...
typename hash_map<T, E, F>::node* hash_map<T, E, F>::node::alc =
    (node*)malloc(sizeof(node));
template <typename T, typename E, typename F = std::hash<T>>
size_t hash_map<T, E, F>::node::cnt = 0;
template <typename T, typename E, typename F = std::hash<T>>
size_t hash_map<T, E, F>::node::alc_sz = 1;

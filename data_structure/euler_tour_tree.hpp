template <typename T, typename E> class euler_tour_tree {
  public:
    struct node;
    using np = node*;
    ...
    bool same(np s, np t) { return root(s) == root(t); }
    ...
    template <typename First, typename... Rest> np merge(First s, Rest... t) {
    ...
    constexpr static E g(E s, E t) { return s + t; }
    constexpr static T h(T s, E t, int sz) { return s + t * sz; }

  public:
    void reroot(int s) { reroot(get_node(s, s)); }

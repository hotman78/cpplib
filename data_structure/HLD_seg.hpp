template <typename T> class HLD_seg {
    class segment {
        T* node;
        int n = 1;
  ...
  public:
    ...
    inline void update(auto t, T x) { seg->update(vertex[t], x); }
    T get(auto u, auto v) {
        T res = e;
        while (head[u] != head[v]) {
  ...
  protected:
    ...
    constexpr static T f(const T& s, const T& t) { return s + t; }
};
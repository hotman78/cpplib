template <typename T, typename F> class UF_data {
  public:
    vector<int> data;
    vector<T> val;
    int sz;
  ...
  public:
    UF_data(int sz, F merge = F()) : sz(sz), merge(merge) {
        data.resize(sz, -1);
        val.resize(sz, T());
    }
    ...
    template <typename G> void apply(int x, G g = G()) {
        x = root(x);
        g(val[x]);
    }
    inline T& get(int x) { return val[root(x)]; }

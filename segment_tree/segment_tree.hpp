...
template <typename T, typename F> class segment_tree {
  ...
  public:
    ...
    optional<T> get(i64 a, i64 b) { return get(a, b, 0, n, root); }
    void apply(i64 x, T val) { apply(x, val, 0, n, root); }
    void apply_left(i64 x, T val) { apply_left(x, val, 0, n, root); }
    void change(i64 x, T val) { change(x, val, 0, n, root); }
    ...
    optional<T> apply(i64 x, T val, i64 l, i64 r, np t) {
        ...
        if (r - l > 1) {
            ...
            return t->val = __op(t->ch[0] ? t->ch[0]->val : nullopt,
                                 t->ch[1] ? t->ch[1]->val : nullopt);
        } else {
...
template <typename T> struct MAX_F {
    T operator()(const T& s, const T& t) { return max(s, t); }
};
template <typename T> using max_seg = segment_tree<T, MAX_F<T>>;

template <typename T> struct MIN_F {
    T operator()(const T& s, const T& t) { return min(s, t); }
};
template <typename T> using min_seg = segment_tree<T, MIN_F<T>>;

template <typename T> struct COMPOSITE_F {
    pair<T, T> operator()(const pair<T, T>& s, const pair<T, T>& t) {
        return make_pair(s.first * t.first, s.second * t.first + t.second);
    }
};
template <typename T>
using composite_seg = segment_tree<pair<T, T>, COMPOSITE_F<T>>;
}  // namespace hotman
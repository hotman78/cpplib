...
class lazy_segment_tree {
    ...
    maybe<T> update(i64 a, i64 b, E x, i64 l, i64 r, np t) {
        ...
        // 一部区間内
        return t->val = f(update(a, b, x, l, (l + r) / 2, t->lch),
                          update(a, b, x, (l + r) / 2, r, t->rch));
    }
    maybe<T> get(i64 a, i64 b, i64 l, i64 r, np t) {
        ...
        // 一部区間内
        return f(get(a, b, l, (l + r) / 2, t->lch),
                 get(a, b, (l + r) / 2, r, t->rch));
    }
    ...
    maybe<T> h(const maybe<T>& s, const maybe<E>& t, i64 l, i64 r) {
        if (t.is_none())
            ...
            return maybe<T>(_h(s, t.unwrap(), l, r));
    }

  public:
    lazy_segment_tree(i64 sz, F f = F(), G g = G(), H h = H())
        : n(1), sz(sz), _f(f), _g(g), _h(h) {
        while (n < sz) n <<= 1;
    }
    // 0-indexed [a,b)
